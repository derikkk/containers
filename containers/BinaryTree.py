...
These two classes are the bu
ilding blocks for the BST, AVLTree, and Heap data structures.
...
class Node():
    ...
    Given a node t, you can visualize th
    e node by running str(t) in the python interpreter.
...
class BinaryTree():
    ...
    def print_tree(self, traversal_type):
        ...
        s of tree traversals: preorder, inorder, and postorder.
        All three of these traversals are implemented
        for you as a reference on how
    ...
    def to_list(self, traversal_type):
        ...
        A general programmin
        g principle is that a function should return its results
        ...
        But by returning the result
        s we can also do more computations on the results if needed.
        Many of the test cases for mor
        e complicated tree functions rely on this to_list function,
    ...
    def __len__helper(node):
        ...
        if a left child exi
        sts, add the result of __len__helper on the left child;
        if a right child exist
        s, add the result of __len__helper on the right child;
    ...
    def height(self):
        ...
        Recall that the height is the maximu
        m length from the root to a leaf node.
